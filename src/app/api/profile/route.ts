...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!user) {
      logApiRequest('GET', '/api/profile', authResult.user.id, 404, Date.now() - startTime);
      return NextResponse.json(
    ...
    logApiRequest('GET', '/api/profile', authResult.user.id, 200, Date.now() - startTime);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (!updatedUser) {
      logApiRequest('PUT', '/api/profile', authResult.user.id, 404, Date.now() - startTime);
      return NextResponse.json(
        { success: false, error: 'User not found' },
        { status: 404 }
      );
    ...
    logApiRequest('PUT', '/api/profile', authResult.user.id, 200, Date.now() - startTime);
