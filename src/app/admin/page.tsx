...
import { CheckCircle, XCircle, Clock, Eye, FileText, User, Users, Building, Calendar } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleStatusUpdate = async (providerId: string, status: string, rejectionReason?: string) => {
    try {
      const token = localStorage.getItem('token');
                ...
                {activities.map((activity, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                    <div className="flex-shrink-0">
                      ...
                      {activity.action === 'register' && <Users className="h-4 w-4 text-blue-500" />}
                      {activity.action === 'listing_create' && <Building className="h-4 w-4 text-purple-500" />}
                      {activity.action === 'booking_create' && <Calendar className="h-4 w-4 text-orange-500" />}
                      {!['login', 'logout', 'register', 'listing_create', 'booking_create'].includes(activity.action) &&
