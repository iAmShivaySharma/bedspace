...
import { Search, Heart, Calendar, MessageCircle, MapPin, Star, Clock, CheckCircle, XCircle, Building, Wifi, Car, Utensils } from 'lucide-react';
                  ...
                  {activeBookings.slice(0, 3).map((booking: any) => (
                    <div key={booking.id} className="p-4 border rounded-lg hover:shadow-sm transition-shadow">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-gray-500">Move-in: {new Date(booking.moveInDate).toLocaleDateString()}</span>
                        <span className="font-medium">₹{booking.price.toLocaleString()}/month</span>
                      </div>
                    </div>
                  ))}
                  <Button variant="outline" className="w-full" onClick={() => router.push('/seeker/bookings')}>
                    View All Bookings
                  </Button>
                </>
              ) : (
                ...
                {savedListings.slice(0, 3).map((listing: any) => (
                  <div key={listing.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-lg">₹{listing.price.toLocaleString()}/month</span>
